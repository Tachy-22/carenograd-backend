-- FINAL COMPLETE SQL FIX - Guaranteed to work
-- This completely removes all functions and recreates them with zero conflicts

-- 1. DROP ALL POSSIBLE FUNCTION VARIATIONS (brute force approach)
...
-- 2. COMPLETELY FRESH FUNCTION: calculate_daily_allocation
CREATE OR REPLACE FUNCTION calculate_daily_allocation(target_model VARCHAR(100) DEFAULT 'gemini-2.5-flash')
RETURNS TABLE (
...
BEGIN
    ...
    WHERE uts.model_name = target_model
    ...
    IF v_active_users = 0 THEN
        ...
        WHERE tur.model_name = target_model
    ...
    WHERE sdt.model_name = target_model
    ...
    IF v_total_requests IS NULL THEN
        ...
        FROM token_pools tp
        WHERE tp.model_name = target_model
...
-- 3. COMPLETELY FRESH FUNCTION: get_user_daily_allocation
CREATE OR REPLACE FUNCTION get_user_daily_allocation(
    target_user_id UUID,
    target_model VARCHAR(100) DEFAULT 'gemini-2.5-flash'
)
RETURNS TABLE (
    user_id UUID,
...
BEGIN
    ...
    FROM calculate_daily_allocation(target_model);
    ...
    AND uts.model_name = target_model
    AND uts.last_reset_day::DATE = CURRENT_DATE;
    ...
    INSERT INTO user_daily_allocations (
        ...
        target_user_id, target_model, CURRENT_DATE,
    ...
    INSERT INTO system_daily_tracking (
        ...
        target_model, CURRENT_DATE, v_allocation_result.total_requests_available,
    ...
    -- Return query with explicit casting
    RETURN QUERY SELECT 
        target_user_id::UUID,
        target_model::VARCHAR(100),
        v_allocated_requests::INTEGER,
        v_user_usage::INTEGER,
        GREATEST(0, v_allocated_requests - v_user_usage)::INTEGER,
        CASE 
            WHEN v_allocated_requests > 0 
            THEN ROUND((v_user_usage::NUMERIC / v_allocated_requests::NUMERIC) * 100, 2)
            ELSE 0::NUMERIC(5,2)
        END,
        (v_user_usage < v_allocated_requests)::BOOLEAN,
        v_allocation_result.active_users::INTEGER;
END;
$$ LANGUAGE plpgsql;

-- 4. COMPLETELY FRESH FUNCTION: record_daily_request_usage
...
BEGIN
    ...
    FROM get_user_daily_allocation(target_user_id, target_model);
    ...
    IF NOT v_current_allocation.can_make_request THEN
        RETURN QUERY SELECT 
            false::BOOLEAN,
            0::INTEGER,
            v_current_allocation.allocated_requests_today::INTEGER,
    ...
    INSERT INTO user_token_usage_summary (
        ...
        target_user_id, target_model, 1, CURRENT_DATE
    )
    ...
    RETURN QUERY SELECT 
        true::BOOLEAN,
        v_current_allocation.requests_remaining_today::INTEGER,
        v_current_allocation.allocated_requests_today::INTEGER,
...
-- Success message
SELECT 'FINAL COMPLETE FIX: All functions recreated with zero conflicts!' as message,
       'Functions now use: target_user_id, target_model parameters' as parameters,
       'Return columns are standard names (no prefixes needed)' as returns;